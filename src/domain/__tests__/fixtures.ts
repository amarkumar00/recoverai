...
export const validSimulatedEvaluation = {
  ...
  datasetVersion: "recoverai-payment-failure-digital-twin-v1",
  datasetFingerprintSha256: "a".repeat(64),
  evaluationPolicyVersion: "recoverai-evaluation-policy-v1",
  baselinePolicyVersion: "generic-payment-link-after-15-minutes-v1",
  ...
  baselineRecoveredCaseCount: 30,
  recoverAiRecoveredCaseCount: 44,
  simulatedRecoveryRate: 0.44,
  rootCauseCorrectCount: 91,
  rootCauseAccuracy: 0.91,
  actionSelectionCorrectCount: 87,
  actionSelectionAccuracy: 0.87,
  unsafeActionsBlocked: 7,
  duplicateEventsIgnored: 13,
  duplicateChargeAttemptsPrevented: 4,
  baselineCustomerContactCount: 50,
  recoverAiCustomerContactCount: 29,
  customerContactsAvoided: 21,
  humanEscalationCount: 5,
  humanEscalationRate: 0.05,
  falsePositiveInterventionCostSimulated: {
    amountSubunits: 50_000,
    currency: "INR",
  },
  baselinePaymentLinkCreationCount: 45,
  paymentLinkCreationCount: 28,
  apiFallbackOrFailureCount: 3,
  meanProcessingTimeMilliseconds: 42.5,
  processingTimeModel: "SIMULATED_DETERMINISTIC_LOGICAL_V1",
  unresolvedExceptionCount: 5,
  unresolvedExceptions: Array.from({ length: 5 }, (_value, index) => ({
    caseReference: `case_unresolved_${index + 1}`,
    strategy: "RECOVERAI",
  ...
  resultsByFailureClass: [
    ...
    "NON_RETRYABLE",
    "AMBIGUOUS",
  ].map((failureClass, index) => ({
    failureClass,
    uniqueCaseCount: index === 0 ? 100 : 0,
    recoveredCaseCount: index === 0 ? 44 : 0,
    rootCauseCorrectCount: index === 0 ? 91 : 0,
    actionSelectionCorrectCount: index === 0 ? 87 : 0,
    simulatedRevenueAtRisk: {
      amountSubunits: index === 0 ? 84_250_000 : 0,
      currency: "INR",
    },
    simulatedRevenueRecovered: {
      amountSubunits: index === 0 ? 36_775_000 : 0,
      currency: "INR",
    },
    simulatedRecoveryRate: index === 0 ? 0.44 : 0,
    rootCauseAccuracy: index === 0 ? 0.91 : 0,
    actionSelectionAccuracy: index === 0 ? 0.87 : 0,
    unresolvedExceptionCount: index === 0 ? 5 : 0,
  })),
  resultsBySelectedAction: [
    ...
    "STOP_NON_RETRYABLE",
    "ESCALATE_HUMAN",
  ].map((selectedAction, index) => ({
    selectedAction,
    caseCount: index === 0 ? 100 : 0,
    recoveredCaseCount: index === 0 ? 44 : 0,
    simulatedRevenueRecovered: {
      amountSubunits: index === 0 ? 36_775_000 : 0,
      currency: "INR",
